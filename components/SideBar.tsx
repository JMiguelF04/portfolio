...
export default function Sidebar({ profile, content, navigationItems, socialLinks }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-72 xl:w-80 flex-col bg-card/90 backdrop-blur-2xl border-r border-border/60 z-50 shadow-[1px_0_60px_rgba(0,0,0,0.5)]">
        {/* Profile */}
        <div className="p-7 text-center border-b border-border/50">
          ...
          <ul className="space-y-1">
            {navigationItems.filter((item) => item.visible).map((item) => (
              <li key={item.href}>
                <a
                  href={item.href}
                  className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-foreground-secondary hover:text-foreground hover:bg-background-secondary/70 transition-all duration-200 group border border-transparent hover:border-border/50"
                >
        ...
        <div className="px-5 pb-4 border-t border-border/50 pt-4">
          <a
            ...
            target="_blank"
            className="flex items-center justify-center gap-2 w-full px-5 py-3 border border-accent/40 text-accent rounded-xl hover:bg-accent hover:text-white hover:border-accent transition-all duration-300 font-semibold text-sm"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex justify-center gap-2.5 mb-4">
            {socialLinks.filter((link) => link.visibleInSidebar).map((link) => (
              <a
                ...
                className="w-9 h-9 flex items-center justify-center rounded-lg border border-border text-foreground-muted hover:border-accent/50 hover:text-accent hover:bg-accent-muted transition-all duration-300"
      ...
      {/* Mobile header */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 glass border-b border-border/60">
            ...
            <div className="flex justify-center gap-3">
              {socialLinks.filter((link) => link.visibleInSidebar).map((link) => (
                <a
                  ...
                  target={link.href.startsWith("http") ? "_blank" : undefined}
                  rel={link.href.startsWith("http") ? "noopener noreferrer" : undefined}
                  className="w-11 h-11 flex items-center justify-center rounded-xl border border-border text-foreground-secondary hover:border-accent/50 hover:text-accent transition-all duration-300"
                  aria-label={link.label}
                >
                  {sidebarIcons[link.platform]}
                </a>
