...
const getLevelColor = (level: string): string => {
  switch (level) {
    ...
    case "Basic":        return "from-yellow-500 to-amber-400";
    ...
    <section id="competencias" className="py-24 bg-background-secondary/40 relative">
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <span className="section-kicker">{t(content, "skills.subtitle")}</span>
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {skillCategories.map((skillGroup) => (
            <div key={skillGroup.id} className="p-7 rounded-3xl bg-card border border-border/70 hover-card">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-lg font-semibold tracking-tight">{skillGroup.labelEn}</h3>
              </div>

              <div className="space-y-3.5">
                {skillGroup.skills.map((skill) => (
                  <div key={skill.id} className="flex items-center justify-between gap-4">
                    <span className="text-foreground-secondary font-medium text-sm">{skill.name}</span>
                    <span className={`text-xs font-semibold bg-gradient-to-r ${getLevelColor(skill.level)} bg-clip-text text-transparent flex-shrink-0`}>
                      {skill.level}
                    </span>
                  </div>
                ))}
        ...
        <div className="mt-14 text-center">
          <p className="text-foreground-muted text-xs mb-7 font-mono tracking-wide uppercase">
            {t(content, "skills.dailyTech")}
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            {dailyTechnologies.map((tech) => (
              <div key={tech.id} className="px-4 py-2 rounded-xl bg-card border border-border/70 text-foreground-secondary text-sm font-medium hover:border-accent/40 hover:text-accent hover:bg-accent-muted transition-all duration-300">
                {tech.name}
              </div>
            ))}
          </div>
